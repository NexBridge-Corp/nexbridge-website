...
const Team = () => {
  ...
  return (
    <Section id="team" subtitle={t.team.subtitle} title={t.team.title}>
      <Container>
        <p className="text-center max-w-2xl mx-auto mb-12 -mt-8" style={{ color: 'var(--color-text)', opacity: 0.6 }}>
          {t.team.description}{' '}
          <span className="text-primary font-medium">{t.team.university}</span>
          {t.team.description2}
        </p>
          ...
          {TEAM_MEMBERS.map((member, i) => (
            ...
            >
              <Card className="text-center group cursor-pointer">
                ...
                <p className="text-primary text-sm font-medium mb-3">{t.team.role}</p>
                <p className="text-sm leading-relaxed mb-4" style={{ color: 'var(--color-text)', opacity: 0.6 }}>
                  {t.team.members[i]?.bio}
                </p>

                {/* LinkedIn icon */}
                <div className="flex justify-center">
                  <span className="inline-flex items-center gap-2 text-xs group-hover:text-primary transition-colors duration-200" style={{ color: 'var(--color-text)', opacity: 0.5 }}>
